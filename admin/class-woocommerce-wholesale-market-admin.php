...
class Woocommerce_Wholesale_Market_Admin {
	...
	public function add_settings_tab( $settings_tabs ) {
		$settings_tabs['wholesalemarket'] = __( 'Wholesale Market', 'woocommerce-wholesale-market' );
	...
	public function get_settings() {
		...
		if ( '' == $current_section ) {
			...
			$settings = array(
				array(
					'title' => __( 'General Setting for wholesale Markeet', 'woocommerce-wholesale-market' ),
					'desc'  => __( 'These pages need to be set so that WooCommerce knows where to send users to inventory_wholesale_options.', 'woocommerce-wholesale-market' ),
					'type'  => 'title',
					'id'    => 'advanced_wholesale_options',
				),
				array(
					'title'           => __( 'Enable/Disble', 'woocommerce-wholesale-market' ),
					'desc'            => __( 'Enable/disable setting', 'woocommerce-wholesale-market' ),
					'id'              => 'woocommerce_enable_disable__setting',
					'default'         => 'no',
					'type'            => 'checkbox',
					'checkboxgroup'   => 'start',
					'show_if_checked' => 'option',
					'desc_tip'        => __( 'Enable/Disable wholesale pricing setting.', 'woocommerce-wholesale-market' ),
				),
				array(
					'title'    => __( 'Show Wholesale Price', 'woocommerce-wholesale-market' ),
					'id'       => 'wholesale_market_prices_show_user_setting',
					'default'  => 'show_all_user',
					'type'     => 'radio',
					'desc_tip' => __( 'This option is important as it will affect how you Show Wholesale price to user.', 'woocommerce-wholesale-market' ),
					'options'  => array(
						'show_all_customer'       => __( 'Show Wholesale Price to All Customers ', 'woocommerce-wholesale-market' ),
						'only_wholesale_customer' => __( 'Show Wholesale Price to Only WholeSale Customers', 'woocommerce-wholesale-market' ),
					),
					'desc_tip' => __( 'Enable/Disable wholesale pricing setting.' ),
				),
				array(
					'title'    => __( 'Wholesale Prefix', 'woocommerce-wholesale-market' ),
					'type'     => 'text',
					'desc'     => __( 'Text Field to store what text should be shown with Wholesale Price.', 'woocommerce-wholesale-marketn' ),
		...
		} elseif ( 'inventory_setting' === $current_section ) {
				$settings = array(
					array(
						'title' => __( 'Inventory Setting for wholesale Markeet', 'woocommerce-wholesale-market' ),
						'desc'  => __( 'These pages need to be set so that WooCommerce knows where to send users to checkout.', 'woocommerce-wholesale-market' ),
						'type'  => 'title',
						'id'    => 'inventory_wholesale_options',
					),

					array(
						'title' => __( 'Enable Minimum Quantity', 'woocommerce-wholesale-market' ),
						'type'  => 'checkbox',
						'desc'  => __( ' Enable min. qty setting for applying wholesale price.', 'woocommerce-wholesale-market' ),
						'id'    => 'enable_minmimum_qunatity_wholesale_settings',

					),
					array(
						'title'    => __( 'Set Min qty', 'woocommerce-wholesale-market' ),
						'id'       => 'wholesale_market_set_min_qty',
						'default'  => 'all_customer',
						'type'     => 'radio',
						'desc_tip' => __( 'This option is important as it will affect how you Show Wholesale price to user.', 'woocommerce-wholesale-market' ),
						'options'  => array(
							'set_min_qty_product_level' => __( 'Set Min qty on product level ', 'woocommerce-wholesale-market' ),
							'set_common_min_qty_all_product' => __( 'Set common min qty for all products.', 'woocommerce-wholesale-market' ),
						),
					),
					array(
						'title'             => __( 'Minimum Quantity', 'woocommerce-wholesale-market' ),
						'type'              => 'number',
						'desc'              => __( 'Text Field to store what text should be shown with Minimum quantity.', 'woocommerce-wholesale-market' ),
		...
		if ( 'yes' == get_option( 'woocommerce_enable_disable__setting', true ) ) {
			...
			woocommerce_wp_text_input(
				array(
					...
					'label'       => __( 'Whole Sale Price!', 'woocommerce-wholesale-market' ) . ' (' . get_woocommerce_currency_symbol() . ')',
					'description' => __( 'Let your customers know when the product wholesale price., unless you define stock at variation level.', 'woocommerce-wholesale-market' ),
					'desc_tip'    => true,
				)
			);
			woocommerce_wp_text_input(
				array(
					...
					'label'       => __( 'Product Quantity', 'woocommerce-wholesale-market' ),
					'description' => __( 'Let your customers know when the product wholesale price., unless you define stock at variation level.', 'woocommerce-wholesale-market' ),
					'desc_tip'    => true,
	...
	public function ced_save_general_wholesale_data( $post_id ) {
		global $post;
		if ( isset( $_POST['wholesale_general_product_nonce'] ) ) {
			if ( wp_verify_nonce( sanitize_text_field( $_POST['wholesale_general_product_nonce'] ), 'create_nonce_for_general_product' ) ) {

				if ( isset( $_POST['general_wholesale_quantity_feild'] ) ) {
					$gen_wholesale_quantity = sanitize_text_field( $_POST['general_wholesale_quantity_feild'] );
					if ( $gen_wholesale_quantity <= 0 ) {
						$this->ced_sample_admin_notice__error( 'Quantity Should greater than 0' );
					} else {
						if ( update_post_meta( $post_id, 'general_wholesale_quantity_feild', esc_attr( $gen_wholesale_quantity ) ) ) {
		...
		if ( 'yes' == get_option( 'woocommerce_enable_disable__setting', true ) ) {
			woocommerce_wp_text_input(
				array(
					...
					'label'         => __( 'Wholesale Quantity ', 'woocommerce-wholesale-market' ),
					'value'         => get_post_meta( $variation->ID, 'variation_wholesale_quantity', true ),
					'wrapper_class' => 'form-row form-row-first',
					'description'   => __( 'Let your customers know when the product wholesale quantity., unless you define stock at variation level.', 'woocommerce-wholesale-market' ),
					'desc_tip'      => true,
				)
			);
			woocommerce_wp_text_input(
				array(
					...
					'label'         => __( 'Wholesale Price ', 'woocommerce-wholesale-market' ),
			...
			woocommerce_wp_text_input(
				array(
					'id'          => 'wholesale_variation_product_nonce[' . $loop . ']',
					'label'       => '',
					'placeholder' => '',
					'type'        => 'hidden',
	...
	public function ced_save_variation_wholesale_data( $variation_id, $i ) {
		global $post;
		echo $_POST['wholesale_variation_product_nonce'][ $i ];
		echo wp_verify_nonce( $_POST['wholesale_variation_product_nonce'][ $i ], 'create_nonce_for_variation_product' );
		die;
		if ( isset( $_POST['variation_wholesale_quantity'][ $i ] ) ) {
			$variation_wholesale_quantity = sanitize_text_field( $_POST['variation_wholesale_quantity'][ $i ] );
			if ( $variation_wholesale_quantity <= 0 ) {
				$this->ced_sample_admin_notice__error( 'variation wholesale should greater then 0' );
			} else {
		...
		if ( isset( $_POST['variation_wholesale_price'][ $i ] ) ) {
			...
			if ( $variation_wholesale_price <= 0 ) {
				$this->ced_sample_admin_notice__error( 'variation should greater then 0' );
			} else {
	...
	public function ced_create_setting_user_to_wholesale_user( $user ) {
		...
		<?php
		wp_nonce_field( 'nonce_for_user_setting', 'nonce_verification_for_user_to_wholeseller' );
	}
	...
	public function ced_save_setting_user_to_wholesale_user_data( $user_id ) {
		if ( isset( $_POST['nonce_verification_for_user_to_wholeseller'] ) ) {
			$nonce = sanitize_text_field( $_POST['nonce_verification_for_user_to_wholeseller'] );
			if ( wp_verify_nonce( $nonce, 'nonce_for_user_setting' ) ) {
	...
	/**
	 * Ced_sample_admin_notice__error
	 * Description : function for shw the error in the form of notices in admin-end
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function ced_sample_admin_notice__error( $data ) {
		$class   = 'notice notice-error is-dismissible';
		$content = $data;
		$message = __( $content, 'woocommerce-wholesale-market' );
		if ( '' != $message ) {

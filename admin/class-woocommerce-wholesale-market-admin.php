...
class Woocommerce_Wholesale_Market_Admin {
	...
	public function ced_create_subsection() {
		...
		foreach ( $sections as $id => $label ) {
			echo '<li><a href="' . admin_url( 'admin.php?page=wc-settings&tab=wholesalemarket&section=' . sanitize_title( $id ) ) . '" class="' . ( $current_section == $id ? 'current' : '' ) . '">' . $label . '</a> ' . ( end( $array_keys ) == $id ? '' : '|' ) . ' </li>';
		}
	 ...
	 * Discription : craete a custom feild for Wholesale price and Whoilesale quantity 
	 * @since 1.0.0
	 * @param  mixed $post_id
	 * @return void
	 */
	public function ced_wholesale_price_quntity_feild($post_id ) {
	 ...
	 * @return void
	 */
	function ced_save_general_wholesale_data( $post_id ) {
	   $gen_wholesale_quantity = $_POST['general_wholesale_quantity_feild'];
	   $gen_wholesale_price = $_POST['general_wholesale_price_feild']; 
	   if ( isset( $gen_wholesale_quantity ) )
	    update_post_meta($post_id, 'general_wholesale_quantity_feild', esc_attr( $gen_wholesale_quantity ) );
	   if ( isset( $gen_wholesale_price ) ) update_post_meta($post_id, 'general_wholesale_price_feild', esc_attr( $gen_wholesale_price ) );
	}
	 ...
	 * @return void
	 */
	function ced_wholesale_price_quantity_feild_for_varition( $loop, $variation_data, $variation ) {
		if('yes' ==get_option( 'woocommerce_enable_disable__setting', true )) {
	 ...
	 * @return void
	 */
	function ced_save_variation_wholesale_data( $variation_id, $i ) {
	   $variation_wholesale_quantity = $_POST['variation_wholesale_quantity'][$i];
	   $variation_wholesale_price = $_POST['variation_wholesale_price'][$i]; 
	 ...
	 * @return void
	 */
	function ced_create_setting_user_to_wholesale_user( $user )
	{
	 ...
	 * @return void
	 */
	function ced_save_setting_user_to_wholesale_user_data( $user_id )
	{			
	    $seting_user_to_wholesale = $_POST['whole_sale_user_checkbox'];
	    if ( isset( $seting_user_to_wholesale ) ) update_user_meta( $user_id, 'whole_sale_user_checkbox', esc_attr( $seting_user_to_wholesale ) );
	 
	}

	function ced_normaluser_to_wholeseller_approval()
	{
		// print_r($_POST);
		$data_id = $_POST['id'];
